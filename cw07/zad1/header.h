...
//semaphore functions
void semaphoreWait(int handle, short sem_num){
    struct sembuf sops;
    sops.sem_flg = 0;
...
int semaphoreCheck(int handle, short sem_num){
    struct sembuf sops;
    sops.sem_flg = IPC_NOWAIT;
    sops.sem_num = sem_num;
    sops.sem_op = -1;
    return semop(handle, &sops, 1);
}

void semaphoreTake(int handle, short sem_num){
    struct sembuf sops;
    sops.sem_flg = 0;
    sops.sem_num = sem_num;
    sops.sem_op = -1;
...
void semaphoreGive(int handle, short sem_num){
    struct sembuf sops;
    sops.sem_flg = 0;
    sops.sem_num = sem_num;
    sops.sem_op = 1;
